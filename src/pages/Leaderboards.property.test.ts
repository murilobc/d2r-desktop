...
const arbArea = fc.constantFrom(
  "Ancient Tunnels", "Chaos Sanctuary", "Pindleskin", "Pit", "Mephisto"
);

// A run that always has duration_secs > 0 and finished_at set (valid for all metrics)
const arbValidRun: fc.Arbitrary<RunWithItemCount> = fc.record({
  id: arbId,
  profile_id: arbId,
  area: arbArea,
...
// A run with finished_at = null (invalid for all metrics)
...
const arbProfile: fc.Arbitrary<Profile> = fc.record({
  ...
  season_start_date: fc.constant(null),
});
    ...
    fc.assert(
      fc.property(
        ...
        (validRuns, invalidRuns) => {
          ...
          expect(bestsOnMixed.fastest_run?.value ?? null).toBe(
            bestsOnValid.fastest_run?.value ?? null
          );
          expect(bestsOnMixed.best_items_in_run?.item_count ?? null).toBe(
            bestsOnValid.best_items_in_run?.item_count ?? null
          );
          expect(bestsOnMixed.longest_run?.value ?? null).toBe(
            bestsOnValid.longest_run?.value ?? null
          );
        }
      ),
      { numRuns: 100 }
...
describe("Property 5: Adding a non-improving run does not change fastest/best-items bests", () => {
  it("adding a run with max duration and 0 items does not change fastest or best-items bests", () => {
    fc.assert(
      fc.property(fc.array(arbValidRun, { minLength: 1, maxLength: 20 }), (runs) => {
        ...
        // fastest_run should be unchanged (our extra run is slower)
        expect(bestsWithExtra.fastest_run?.value ?? null).toBe(
          bests.fastest_run?.value ?? null
        );

        // best_items_in_run: if current best > 0, it should be unchanged
        if (bests.best_items_in_run !== null && bests.best_items_in_run.item_count > 0) {
          expect(bestsWithExtra.best_items_in_run?.item_count ?? null).toBe(
  ...
  it("all non-null run_ids in personal bests exist in the input array", () => {
    fc.assert(
      fc.property(fc.array(arbValidRun, { minLength: 1, maxLength: 20 }), (runs) => {
        ...
        if (bests.fastest_run !== null) expect(ids.has(bests.fastest_run.run_id)).toBe(true);
        if (bests.best_items_in_run !== null) expect(ids.has(bests.best_items_in_run.run_id)).toBe(true);
        if (bests.best_items_per_hour !== null) expect(ids.has(bests.best_items_per_hour.run_id)).toBe(true);
        if (bests.longest_run !== null) expect(ids.has(bests.longest_run.run_id)).toBe(true);
      }),
      { numRuns: 100 }
    );
  });
    ...
    fc.assert(
      fc.property(
        ...
        fc
          ...
          .filter((d) => !isNaN(d.getTime())),
        fc
          .date({ min: new Date("2024-06-01"), max: new Date("2025-01-01") })
          .filter((d) => !isNaN(d.getTime())),
        (duration, earlierDate, laterDate) => {
          const makeRun = (id: string, started: Date): RunWithItemCount => ({
            id,
            profile_id: "p1",
            area: "Pit",
          ...
          const earlier = makeRun("early-id", earlierDate);
          const later = makeRun("late-id", laterDate);
  ...
  it("since-filtered bests equal bests computed on the manually filtered subset", () => {
    fc.assert(
      fc.property(
        ...
        fc
          .date({ min: new Date("2020-01-01"), max: new Date("2024-01-01") })
          .filter((d) => !isNaN(d.getTime())),
        (runs, sinceDate) => {
