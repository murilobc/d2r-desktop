...
export default function Leaderboards({ profile }: Props) {
  ...
  const loadSeasons = useCallback(async () => {
    try {
      ...
      // silently ignore — seasons section shows empty
    }
  }, [profile.id]);
  ...
  function renderDelta(
    ...
    const color = improved ? "#4caf50" : pct === 0 ? "inherit" : "#f44336";
      ...
      <div className="herald-section">
        ...
        {!bestsLoading && !bestsError && !hasAnyBests && (
          <p className="empty-state">
            No records yet — complete some runs to see your personal bests.
          </p>
        )}
        {hasAnyBests && (
        ...
        {comparison && (
          <>
            {comparison.subject_a.total_runs === 0 && (
              <p className="empty-state" style={{ margin: "0 0 0.5rem 0" }}>
            ...
            <table className="stats-table">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    {comparison.subject_a.fastest_run_secs != null &&
                    comparison.subject_b.fastest_run_secs != null
                      ? renderDelta(
                          comparison.subject_a.fastest_run_secs,
                          comparison.subject_b.fastest_run_secs,
      ...
      <div className="herald-section">
            ...
            <tbody>
              {seasons.map((s) => (
                <tr key={s.id}>
                  ...
                  <td>
                    {s.bests_snapshot.fastest_run
                      ? formatDuration(s.bests_snapshot.fastest_run.value)
                      : "—"}
                  </td>
                  <td>
                    {s.bests_snapshot.best_items_in_run
                      ? s.bests_snapshot.best_items_in_run.item_count
                      : "—"}
                  </td>
                  <td>
                    {s.bests_snapshot.best_items_per_hour
                      ? `${s.bests_snapshot.best_items_per_hour.items_per_hour.toFixed(1)}/hr`
                      : "—"}
                  </td>
                </tr>
              ))}
        ...
        {showSeasonDialog && (
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Start New Season"
          >
            <div className="modal-content">
              <h3>Start New Season</h3>
              <p>
                Enter a name for this season archive (e.g., "Ladder Season 7"). Your
                current personal bests will be archived and fresh stats begin.
              </p>
              <input
                type="text"
                value={seasonName}
